...
class HeadlessDisplayer:
    ...
    def display(self, dpflag=Event(), fractionDone=None, timeEst=None,
                ...
                statistics=None, **kws):
        if (self.last_update_time + 0.1 > clock() and fractionDone not in
           (0.0, 1.0) and activity is not None):
            return
        ...
        if statistics is not None:
            if (statistics.shareRating < 0) or (statistics.shareRating > 100):
                ...
                self.shareRating = '%.3f  (%.1f MB up / %.1f MB down)' % (
                    statistics.shareRating,
                    float(statistics.upTotal) / (1 << 20),
                    float(statistics.downTotal) / (1 << 20))
            if not self.done:
                self.seedStatus = '%d seen now, plus %.3f distributed copies'\
                    % (statistics.numSeeds,
                        0.001 * int(1000 * statistics.numCopies))
            else:
                self.seedStatus = '%d seen recently, plus %.3f distributed '\
                    'copies' % (
                        statistics.numOldSeeds,
                        0.001 * int(1000 * statistics.numCopies))
